...
export class OrderBroadcastService {
  ...
  onOrderCreated(callback: (order: Order) => void): () => void {
    console.log('[Order List Broadcast Service] Registering listener for:', this.ORDER_CREATED_EVENT);
    
    const handler = (event: Event) => {
      console.log('[Order List Broadcast Service] Event received!', event);
      const customEvent = event as CustomEvent<Order>;
      console.log('[Order List Broadcast Service] Custom event detail:', customEvent.detail);
      callback(customEvent.detail);
    };
    
    window.addEventListener(this.ORDER_CREATED_EVENT, handler);
    console.log('[Order List Broadcast Service] Listener registered on window successfully');
    
    // Return cleanup function
    return () => {
      console.log('[Order List Broadcast Service] Removing listener');
